...
### 4.1 Detect Monorepo Tool

```bash
if [ -f nx.json ]; then echo "nx"; elif [ -f turbo.json ]; then echo "turbo"; else echo "standard"; fi
```

### 4.2 Dry Run Validation Commands

Test that the validation commands work (without expecting them to pass):

```bash
# Check type-check (monorepo-aware)
if [ -f nx.json ]; then
  bun nx affected -t typecheck --help 2>/dev/null || echo "nx typecheck not available"
else
...
# Check test (monorepo-aware)
if [ -f nx.json ]; then
  bun nx affected -t test --help 2>/dev/null || echo "nx test not available"
else
  bun run test --help 2>/dev/null || echo "test not available"
fi
```

### 4.3 Document Command Availability

| Build System | Type Check | Tests | Lint |
|---|---|---|---|
| **nx** | `bun nx affected -t typecheck` | `bun nx affected -t test` | `bun run lint` |
| **turbo/standard** | `bun run type-check` | `bun run test` | `bun run lint` |
...
**Monorepo-aware**: If `nx.json` exists, use nx affected targets for type-check and tests.
