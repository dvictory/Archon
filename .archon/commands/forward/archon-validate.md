...
### 1.3 Detect Monorepo Tool

```bash
if [ -f nx.json ]; then echo "nx"; elif [ -f turbo.json ]; then echo "turbo"; else echo "standard"; fi
```

**Use monorepo-aware commands throughout this phase:**
- **nx**: Type check → `bun nx affected -t typecheck`, Tests → `bun nx affected -t test`
- **turbo/standard**: Type check → `{runner} run type-check`, Tests → `{runner} run test`
- **Lint**: Always `{runner} run lint` (root-level)

### 1.4 Determine Available Commands
...
```bash
# Monorepo-aware: nx uses affected targets
if [ -f nx.json ]; then {runner} nx affected -t typecheck; else {runner} run type-check; fi
```
...
```bash
# Monorepo-aware: nx uses affected targets
if [ -f nx.json ]; then {runner} nx affected -t test; else {runner} run test; fi
```
