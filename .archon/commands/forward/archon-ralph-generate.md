...
Every story must pass (monorepo-aware — if `nx.json` exists, use nx affected targets):
- [ ] Type-check: if nx → `bun nx affected -t typecheck`, else → `bun run type-check`
- [ ] Lint: `bun run lint` (always root-level)
- [ ] Tests: if nx → `bun nx affected -t test`, else → `bun run test`
