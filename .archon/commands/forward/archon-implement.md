...
Common patterns:
- JS/TS: if nx → `{runner} nx affected -t typecheck`, else → `{runner} run type-check`
- Python: `mypy .`
- Rust: `cargo check`
- Go: `go build ./...`
...
Common patterns:
- JS/TS: if nx → `{runner} nx affected -t typecheck`, else → `{runner} run type-check`; then `{runner} run lint`
...
Common patterns:
- JS/TS: if nx → `{runner} nx affected -t test`, else → `{runner} run test`
